...
func TestSagaConsumer_OrderConfirmed_SetsConfirmed(t *testing.T) {
	store := NewStore()
	store.Create(&order.Order{OrderId: "abc", Status: order.OrderStatus_ORDER_STATUS_PENDING})
	c := NewSagaConsumer(store)

	err := c.Handle(context.Background(), kafkago.Message{Value: envelope(t, "abc", "order.confirmed")})
	if err != nil {
...
func TestSagaConsumer_OrderRejected_SetsRejected(t *testing.T) {
	...
	err := c.Handle(context.Background(), kafkago.Message{Value: envelope(t, "abc", "order.rejected")})
	if err != nil {
		t.Fatalf("Handle err = %v", err)
	}
...
func TestSagaConsumer_UnknownEvent_NoError_NoChange(t *testing.T) {
	...
	err := c.Handle(context.Background(), kafkago.Message{Value: envelope(t, "abc", "something.else")})
	if err != nil {
